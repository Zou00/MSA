...
def train():
    ...
    for e in range(epoch):
        ...
        if metrics["avg_f1"] > best_avg_f1:
            ...
            save_model(config.output_path, f"{config.fuse_model_type}_best_avg_f1", model)
            print('更新最佳平均F1模型!')
            
        if metrics["weighted_f1"] > best_weighted_f1:
            best_weighted_f1 = metrics["weighted_f1"]
            save_model(config.output_path, f"{config.fuse_model_type}_best_weighted_f1", model)
            print('更新最佳加权F1模型!')
        print()
